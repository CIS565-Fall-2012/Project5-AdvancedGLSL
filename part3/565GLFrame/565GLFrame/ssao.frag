...
float gatherOcclusion( vec3 pt_normal,
	...
	float dist = length(pt_position - occluder_position);
	if(dist < falloff)
	{
		float dir = dot(pt_normal, normalize(occluder_position - pt_position));
		if(dir > 0.0)
		{
			return max(0.0,3*dir*(1.0f / (1.0f + dist)));
		}
		else
		{
			return 0.0f;
...
float occlusionWithRegularSamples(vec2 texcoord, 
	...
	int size = 4;
	vec3 pt_position = position;
	vec3 pt_normal = normal;
	...
	for(int i = 0; i < size; i++)
	{
		...
		{
			vec2 Frag = texcoord + vec2(REGULAR_SAMPLE_STEP*(i-1), REGULAR_SAMPLE_STEP*(j-1));
			if(i == 1 && j == 1)		//Same fragment
			{	
				//occlusionSum += 0.0f;
			}
			else
			{
				vec3 oc_normal = sampleNrm(Frag);
				vec3 oc_position = samplePos(Frag);
					
				occlusionSum += gatherOcclusion(pt_normal, pt_position, oc_normal, oc_position);
			}
		}
	}
	return occlusionSum / 16.0f;
}
...
float occlusionWithPoissonSSSamples(vec2 texcoord, 
	...
	for(int i = 0; i < size; i++)
	{
		vec2 Frag = texcoord + SS_RADIUS * poissonDisk[i];
...
float occlusionWithWorldSpaceSamples(vec2 texcoord,
	...
	for(int i = 0; i < size; i++)
	{
		...
		vec3 oc_position = pt_position + SPHERE_RADIUS * poissonSphere[i];
		vec4 n = (u_Persp * vec4(oc_position, 1.0));
		n = n / n.w;
		vec2 nor = n.xy;
		nor = clamp(nor, vec2(-1.0), vec2(1.0));
		nor = (nor + vec2(1.0)) / 2.0;
		vec3 oc_normal = sampleNrm(n.xy);
