...
float gatherOcclusion( vec3 pt_normal,
	...
	float dist = abs(length(occluder_position - pt_position));
    float over = max(0.0,dot( pt_normal, normalize( occluder_position - pt_position ) ));
    float normDiff = 1.0 - abs( dot( pt_normal, occluder_normal ) );

	float occ = normDiff * over * (1.0/(1.0 + dist));
	
	return occ;
}

const float REGULAR_SAMPLE_STEP = 0.012f;
const float NUM_OF_SAMPLES = 16.0f;
float occlusionWithRegularSamples(vec2 texcoord, 
	vec3 position,
    vec3 normal) {
	
	float depthDifference, occlusionSum = 0.0;
	vec3 ray, occPos, occNorm;
    for( int i = -2; i <= 2; i++ )
	{
		for( int j = -2; j <= 2; j++ )
        {				
			if(i == 0 || j == 0 || i == j || i == -j)
			{
				ray = REGULAR_SAMPLE_STEP * reflect(vec3(i,j,0),getRandomNormal(texcoord));
				occPos = position + sign(dot(ray,normal)) * ray;
				occNorm = getRandomNormal(occPos.xy);
				
				//occNorm =  texture2D(u_Normaltex,occPos.xy).xyz;
				//occlusionSum += step(falloff,depthDifference) * gatherOcclusion(normalize(normal),position,normalize(occNorm),occPos);

				occlusionSum += gatherOcclusion(normalize(normal),position,normalize(occNorm),occPos);

			}
		}
	}

    return occlusionSum / NUM_OF_SAMPLES;
}
