...
#define	DISPLAY_PULSE 5
#define	DISPLAY_SKINNING 6
...
float gatherOcclusion( vec3 pt_normal,
	...
	float actualDist = distance(pt_position, occluder_position);
	if(actualDist > 0.3)
	{
		return 0.0;
	}
	...
	{
		float directionFactor = max(dot(normalize(occluder_position - pt_position), pt_normal), 0.0);
		float distanceFactor = 1.0 / (1.0 + actualDist);
		return directionFactor * distanceFactor;
	}
}
...
float occlusionWithRegularSamples(vec2 texcoord, 
    ...
    vec3 normal) {
	//return -1.0f; //IMPLEMENT THIS
	//vec3 randNormal = getRandomNormal(texcoord);  //(getRandomScalar(texcoord), getRandomScalar(texcoord));
	//float rad = REGULAR_SAMPLE_STEP / linearizeDepth(p.z, u_Near, u_Far);

	float a0 = 0.0;
	vec2 newTex = vec2(-1.5 * REGULAR_SAMPLE_STEP  + texcoord.x, -1.5 * REGULAR_SAMPLE_STEP + texcoord.y);
	for(int i = 0; i < 4; ++i)
	{
		newTex.y = -1.5 * REGULAR_SAMPLE_STEP + texcoord.y;
		for(int j = 0; j < 4; ++j)
		{
			a0 += gatherOcclusion(normal, position, sampleNrm(newTex), samplePos(newTex));
			newTex.y += REGULAR_SAMPLE_STEP;
		}
		newTex.x += REGULAR_SAMPLE_STEP;
	}
	a0 /= 16.0;
	return a0;
}
...
float occlusionWithPoissonSSSamples(vec2 texcoord, 
	...
	{
		newTex = texcoord + SS_RADIUS * poissonDisk[i];
		a0 += gatherOcclusion(normal, position, sampleNrm(newTex), samplePos(newTex));
	}
	a0 /= 16.0;
	return a0;
