...
device_mesh_t uploadMesh(const mesh_t & mesh) {
	...
	//Upload bone information
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, out.bone);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, mesh.bone.size()*sizeof(GLuint), &mesh.bone[0], GL_STATIC_DRAW);
...
void initMesh() {
	...
	while(f<totalsize){
		...
		for(int i=f; i<process+f; i++){
			...
			if(points > 1500)
			{
				mesh.bone.push_back(0);
			}
			else
			{
				mesh.bone.push_back(1);
			}
			points = points + 3;
		}
...
void initPass() {
	...
	glBindAttribLocation(pass_prog,mesh_attributes::BONE, "boneNumber");
...
void initPulse() {
	...
	glBindAttribLocation(pulse_prog, quad_attributes::POSITION, "Position");
	glBindAttribLocation(pulse_prog, quad_attributes::TEXCOORD, "Texcoord");
...
void draw_mesh() {
	...
	mat4 model2 = glm::translate(model, glm::vec3(10000, 0, 0));
	...
	glUniformMatrix4fv(glGetUniformLocation(pass_prog,"u_Model2"),1,GL_FALSE,&model2[0][0]);
	...
	glUniform1f(glGetUniformLocation(pass_prog, "u_time"), time);
	glUniform1i(glGetUniformLocation(pass_prog, "u_effect"), display_type);

	for(int i = 0; i<draw_meshes.size(); i++){
		////Rough Skinning
		//if(i < draw_meshes.size() / 2)
		//{
		//	glUniform1i(glGetUniformLocation(pass_prog, "boneNumber"), 0);
		//}
		//else
		//{
		//	glUniform1i(glGetUniformLocation(pass_prog, "boneNumber"), 1);
		//}
		glBindVertexArray(draw_meshes[i].vertex_array);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, draw_meshes[i].vbo_indices);

		//glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, draw_meshes[i].bone);
...
void draw_quad() {
    ...
    glUniform1i(glGetUniformLocation(pulse_prog, "u_ScreenHeight"), height);
    glUniform1i(glGetUniformLocation(pulse_prog, "u_ScreenWidth"), width);
    glUniform1f(glGetUniformLocation(pulse_prog, "u_Far"), FARP);
    glUniform1f(glGetUniformLocation(pulse_prog, "u_Near"), NEARP);
    glUniform1i(glGetUniformLocation(pulse_prog, "u_OcclusionType"), occlusion_type);
    glUniform1i(glGetUniformLocation(pulse_prog, "u_DisplayType"), display_type);
    glUniformMatrix4fv(glGetUniformLocation(pulse_prog, "u_Persp"),1, GL_FALSE, &persp[0][0] );
	//glUniform1f(glGetUniformLocation(pulse_prog, "u_time"), time);
    
	glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, depthTexture);
    glUniform1i(glGetUniformLocation(pulse_prog, "u_Depthtex"),0);
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, normalTexture);
    glUniform1i(glGetUniformLocation(pulse_prog, "u_Normaltex"),1);
    glActiveTexture(GL_TEXTURE2);
    glBindTexture(GL_TEXTURE_2D, positionTexture);
    glUniform1i(glGetUniformLocation(pulse_prog, "u_Positiontex"),2);
    glActiveTexture(GL_TEXTURE3);
    glBindTexture(GL_TEXTURE_2D, random_normal_tex);
    glUniform1i(glGetUniformLocation(pulse_prog, "u_RandomNormaltex"),3);
    glActiveTexture(GL_TEXTURE4);
    glBindTexture(GL_TEXTURE_2D, random_scalar_tex);
    glUniform1i(glGetUniformLocation(pulse_prog, "u_RandomScalartex"),4);
...
void updateDisplayText(char * disp) {
	switch(display_type) {
			...
			case(DISPLAY_TOTAL):
				...
				break;
			case(DISPLAY_PULSE):
				sprintf(disp, "Displaying Vertex Pulsing");
				break;
			case(DISPLAY_SKINNING):
				sprintf(disp, "Displaying Vertex Skinning");
				break;
	}
}
...
void keyboard(unsigned char key, int x, int y) {
	...
	case('0'):
	  ...
	  //Pulse
	case('p'):
		display_type = DISPLAY_PULSE;
		break;
	case('S'):
		display_type = DISPLAY_SKINNING;
		break;
}
