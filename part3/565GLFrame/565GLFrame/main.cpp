...
GLuint bulge_prog;
void initBulge() {
	Utility::shaders_t shaders = Utility::loadShaders("bulge.vert", "pass.frag");

	bulge_prog = glCreateProgram();

	glBindAttribLocation(bulge_prog,mesh_attributes::POSITION, "Position");
	glBindAttribLocation(bulge_prog,mesh_attributes::NORMAL, "Normal");

	Utility::attachAndLinkProgram(bulge_prog,shaders);
	
}

GLuint keyframe_prog;
void initKeyframe() {
	Utility::shaders_t shaders = Utility::loadShaders("keyframe.vert", "pass.frag");

	keyframe_prog = glCreateProgram();

	glBindAttribLocation(keyframe_prog,mesh_attributes::POSITION, "Position");
	glBindAttribLocation(keyframe_prog,mesh_attributes::NORMAL, "Normal");

	Utility::attachAndLinkProgram(keyframe_prog,shaders);
...
void initFBO(int w, int h) {
    ...
    GLint normal_loc = glGetFragDataLocation(current_prog,"out_Normal");
    GLint position_loc = glGetFragDataLocation(current_prog,"out_Position");
    GLenum draws [2];
...
void draw_mesh() {
	...
	if(current_prog==bulge_prog) {

	/*frequency=(float)(frame*50/(currenttime-timebase));*/
	frequency=0.5f;
	time+=0.1f;

	glUniform1f(glGetUniformLocation(current_prog, "u_time"), time);
	glUniform1f(glGetUniformLocation(current_prog, "u_frequency"), frequency);
	}

	if(current_prog==keyframe_prog) {
...
enum SHADER shader_type =SHADER_PASS;
...
void updateShaderText(char * disp) {
	switch(shader_type) {
			case(SHADER_PASS):
				sprintf(disp, "with Passthrough Shacer");
				break;
			case(SHADER_BULGE):
				sprintf(disp, "with Bulging Shacer");
				break;
			case(SHADER_KEYFRAME):
				sprintf(disp, "with Keyframe Shader");
				break;
	}
}
...
//calculate the frames per second
	...
	if (currenttime - timebase > 1000) 
	{
		...
		sprintf(title, "CIS565 OpenGL Frame | %s %s %s FPS: %4.2f", disp, occl,sha,frame*1000.0/(currenttime-timebase));
...
void keyboard(unsigned char key, int x, int y) {
	...
	case('9'):
	  ...
	  break;
	case('z'):
	  shader_type = SHADER_BULGE;
      current_prog = bulge_prog;
	  break;
	case('x'):
      current_prog=keyframe_prog;
	  shader_type = SHADER_KEYFRAME;
	  break;  
}
