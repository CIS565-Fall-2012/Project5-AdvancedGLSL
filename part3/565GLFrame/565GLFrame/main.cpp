...
mat4x4 get_mesh_world() {
	vec3 tilt(0.0f,1.0f,0.0f);
	mat4 translate_mat = glm::translate(glm::vec3(0.0f,0.0f,0.0f));
...
void draw_mesh() {
	...
	glUniform1f(glGetUniformLocation(pass_prog,"u_time"), (float)frame);

	for(int i=0; i<(int)draw_meshes.size(); i++){
